...
import dev.md19303.lab8.Adapter.Adapter_Item_District_Select_GHN;
import dev.md19303.lab8.Adapter.Adapter_Item_Province_Select_GHN;
import dev.md19303.lab8.Adapter.Adapter_Item_Ward_Select_GHN;
...
import dev.md19303.lab8.Service.GHNRequest;
import dev.md19303.lab8.Service.GHNServices;
import dev.md19303.lab8.databinding.ActivityLocationBinding;
...
public class LocationActivity extends AppCompatActivity {
    ...
    private void sendOrderToGHN() {
        ...
        request.callAPI().createOrder(orderRequest).enqueue(new Callback<ResponseGHN<OrderResponse>>() {
            @Override
            public void onResponse(Call<ResponseGHN<OrderResponse>> call, Response<ResponseGHN<OrderResponse>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    if (response.body().getCode() == 200) {
                        // Thông báo thành công
                        Toast.makeText(LocationActivity.this, "Đặt hàng thành công!", Toast.LENGTH_SHORT).show();
                    } else {
                        // Xử lý lỗi từ server GHN
                        Toast.makeText(LocationActivity.this, "Lỗi: " + response.body().getMessage(), Toast.LENGTH_SHORT).show();
                    }
                } else {
                    // Xử lý lỗi HTTP
                    Toast.makeText(LocationActivity.this, "Gửi đơn hàng thất bại!", Toast.LENGTH_SHORT).show();
                    Log.e("LocationActivity", "Lỗi HTTP: " + response.code() + " -");
                }
            }

            @Override
            public void onFailure(Call<ResponseGHN<OrderResponse>> call, Throwable t) {
                // Xử lý lỗi kết nối
